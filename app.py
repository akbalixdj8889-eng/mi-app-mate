...
# --- PANTALLA 3: FEEDBACK (Este bloque va pegado al borde izquierdo, fuera de 'examen') ---
    ...
    # Lógica de avance a Misión 2 (o no) - ESTO ES PARA MISION 1
    if st.session_state.mision == 1 and puntaje < 3:
        st.error("No has logrado los aciertos mínimos para la Misión 2.")
        # Aquí podrías añadir un botón para "Intentar Misión 1 de Nuevo" si quisieras
    else:
        # Si se aprobó Misión 1 (o si ya estábamos en Misión 2)
        # Aquí es donde aplicamos el criterio para Misión 2
        
        # Verificamos si estamos en Misión 1 y si cumplimos para avanzar (esto ya está implícito o explícito)
        # O si ya estamos en Misión 2, aplicamos el criterio de aprobación para MISIÓN 2
        
        # Lógica unificada para el éxito:
        # Se considera éxito si:
        # 1. Se completó Misión 1 (st.session_state.mision == 1) AND puntaje >= 3
        # 2. Se completó Misión 2 (st.session_state.mision == 2) AND puntaje >= 3
        ...
        # Si es la misión final (Misión 2) y se aprueba, se muestra éxito
        if st.session_state.mision == 2 and aprobado:
            st.balloons()
            st.success("¡Misión 2 Cumplida! ¡Felicidades, Cadete Maestro!")
            # Aquí puedes enviar los datos finales a Google
            # enviar_a_google(st.session_state.nombre, st.session_state.curso, st.session_state.mision, puntaje, st.session_state.power_5050) # Ejemplo
        elif st.session_state.mision == 1 and puntaje >= 3:
            st.success("¡Has superado la Misión 1! ¡Listo para la Misión 2!")
            # No mostramos balloons aquí porque es una misión intermedia
        else: # Si no se cumplieron los criterios
            if st.session_state.mision == 1: # Ya manejado arriba con st.error
                pass # El mensaje de error ya está puesto
            elif st.session_state.mision == 2: # Si se falla Misión 2
                st.error("No has alcanzado la puntuación mínima para aprobar la Misión 2.")
            
    # Botón de reiniciar (este permanece igual)
    if st.button("INTENTAR DE NUEVO"):
        st.session_state.update({
            'paso': 'registro',
            'mision': 1,
            ...
            'usar_5050': False,
            'lista_examen': [],
            'datos_enviados': False # Asegúrate que esto se resetee si lo usas
        })
        st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
